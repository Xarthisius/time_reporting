...
def process_args():
    ...
    action.add_argument(
        '--csv', help='Format: date,M,T,W,R,F '
    ...
    with open(infile, 'r') as f:
        for l in f:
            ...
            if len(weekday_hours) != 5:
                logging.warn("expected 5 workdays, got {0}".format(
                    len(weekday_hours)))
                continue
...
def run(args):
    data = {}
    reporter = time_reporter.Time_Reporter(
    ...
    for key in sorted(overdue):
        ...
        if key in data:
            ...
            if not args.dryrun:
                ...
                logging.info(
                    'Successfully submitted week: '
                    '{0} hours:{1}'.format(key, data[key]))
            if args.once:
                raise SystemExit()
