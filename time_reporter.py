...
class Time_Reporter(object):
    ...
    # IS ALWAYS TRACKED. THIS ALLOWS A SIMPLE FORM OF LAZY LOADING AND
    # SHOULD HELP PREVENT MULTIPLE RELOADS OF THE SAME PAGE.
    ...
    def _load_date(self, date):
        ...
        self.g.doc.text_assert(
            'Enter time for the week starting {0}'.format(date_str))

    def get_overdue_weeks(self):
        '''
        returns dict with key = Python datetime.date object and value = actual
        value (ie: month=5&selectedWeek=05/01/2016&CurrentWkYear=2016 )
        ...
        for elem in self.g.doc.form.inputs:
            if elem.name == 'pastDueWeek':
                for v in elem.value_options:
                    ...
                    overdue_date = datetime.datetime.strptime(
                        datestr, self.DATE_FORMAT)
    ...
    def submit(self, date, hours):
        ...
        Date should be a Python date or datetime object.  Hours should be a
        Python list object (of length 5 or 7, all values must be integers).
        ...
        if len(hours) != 7:
            raise UserWarning(
                "Expected either 5 or 7 hour values, "
                "got '{0}'".format(len(hours)))
        ...
        for k, v in totals_data.items():
            _LOGGER.debug(
                "Attempt to set form item '{0}' to '{1}'".format(k, v))
            self.g.doc.set_input(k, v)
        date_str = date.strftime(self.DATE_FORMAT)
        lp = 'SUBMIT_DATE_{0}'.format(date_str)
        self._submit(last_page=lp)
        assert_str = (
            'You have successfully submitted your time spent on '
            'University business for the week of {0}.'.format(date_str))
